...
def run_for_epochs(example_path, files, model_class, conf_dict, enable_saving=False, epochs=1):
...
def test(example_path, files, model_class, run_once=False, enable_saving=False, epochs=1):
    conf_dict = oneshot.base_conf_dict
    if run_once:
        run_for_epochs(example_path, files, model_class, conf_dict=conf_dict,
        ...
        for conf in conf_dict_list:
            avg_score = run_for_epochs(example_path, files, model_class, conf_dict=conf,
                                       enable_saving=enable_saving, epochs=epochs)
...
# test(const.EXAMPLE_FILE, file_list, oneshot.OneShotTestT2TModel, run_once=True, epochs=1)
